...
def is_match(document):
    if "NATO" or "North Atlantic Treaty Organization" in document["text"]:
        if _contains_any_term(["spending", "contribution", "percent of gdp", "defense budget"], document["text"].lower()): 
            return True
    return False
